...
rofl_result_t processing_init_eventdev(void){
	...
	for (unsigned int lcore_id = 0; lcore_id < rte_lcore_count(); lcore_id++) {
		...
		if (lcores[lcore_id].is_rx_lcore) {
			...
			XDPD_DEBUG(DRIVER_NAME"[processing][init][evdev] eventdev %s, ev_port_id: %2u, rx lcore %3u\n", eventdev_name.c_str(), port_id, lcore_id);
		...
		if (lcores[lcore_id].is_tx_lcore) {
			...
			XDPD_DEBUG(DRIVER_NAME"[processing][init][evdev] eventdev %s, ev_port_id: %2u, tx lcore %3u, linked to ev_queue_id: %2u\n",
		...
		if (lcores[lcore_id].is_wk_lcore) {
			...
			XDPD_DEBUG(DRIVER_NAME"[processing][init][evdev] eventdev %s, ev_port_id: %2u, wk lcore %3u, linked to ev_queue_id: %2u\n",
...
int processing_packet_reception(void* not_used){
	...
	if (task->nb_rx_queues == 0) {
		RTE_LOG(INFO, XDPD, "rx-task-%02u: task has no rx-queues assigned, terminating\n", lcore_id);
		return ROFL_SUCCESS;
	}
	...
	for (i = 0; i < task->nb_rx_queues; i++) {
		...
		RTE_LOG(INFO, XDPD, "rx-task-%02u:  port-queue mappings => port_id=%hhu rx_queue_id=%hhu\n", lcore_id, port_id, queue_id);
	}
